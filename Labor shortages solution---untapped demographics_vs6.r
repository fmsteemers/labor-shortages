...
### Read DDI file
ddi_vector <- c("cps_00101.xml","cps_00104.xml","cps_00108.xml")
...
# Reference labels
reference_labels <- ddi_list$cps_00108.xml$var_info$val_labels
names(reference_labels) <- ddi_list$cps_00108.xml$var_info$var_name
...
emp_base_new <- emp_base_temp %>%
  select(year,month,wtfinl,occ2010_2digit,sex,diffany,educ_b,age_group,race_group,empstat,labforce,nilfact) %>%
  group_by(year,month,occ2010_2digit,sex,diffany,educ_b,age_group,race_group,empstat,labforce,nilfact) %>%
  summarise(pop = sum(wtfinl)) %>%
  #summarise(emp = n()) %>%
  ungroup() %>%
...
emp_base_new_6d <- emp_base_temp %>%
  select(year,month,wtfinl,occ2010_2digit,sex,diffany,educ_b,age_group,race_group,empstat,labforce,nilfact,occ2010) %>%
  group_by(year,month,occ2010_2digit,sex,diffany,educ_b,age_group,race_group,empstat,labforce,nilfact,occ2010) %>%
  summarise(pop = sum(wtfinl)) %>%
  #summarise(emp = n()) %>%
  ungroup() %>%
  mutate(empstat_adj = ifelse(empstat==34 & nilfact==1,32,empstat)) %>%
...
write_csv(emp_base_new_6d %>% select(year,month,occ2010,occ2010_lbl,occ2010_2digit,educ_b,age_group,race_group,empstat_adj_lbl,sex_lbl,pop,labforce_lbl),'emp_base_new_6d.csv')
